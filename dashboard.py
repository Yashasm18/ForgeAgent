...
<script>async function load(){let [r,e,b]=await Promise.all([fetch('/api/tools'),fetch('/api/events'),fetch('/api/benchmark')]),d=await r.json(),a=await e.json(),q=await b.json(),t=d.tools||[];count.textContent=t.length;reuses.textContent=t.reduce((a,x)=>a+(x.reuse_count||0),0);tests.textContent=t.reduce((a,x)=>a+(x.tests||[]).length,0);safety.textContent=`${q.passed}/${q.total}`;tools.innerHTML=t.length?t.map(x=>`<article class="tool"><div><code>${x.name}</code><p>${x.description}</p><p>Provenance: ${x.provenance||'legacy demo'} · ${x.tests?.length||1} proof case(s)</p></div><span class="badge">TRUSTED</span></article>`).join(''):'<div class="empty">No tools have earned trust yet. Forge your first capability.</div>';events.innerHTML=(a.events||[]).map(x=>`<article class="tool"><div><code>${x.event}</code><p>${x.capability} — ${x.detail}</p><p>${x.created_at}</p></div><span class="badge">${x.outcome.toUpperCase()}</span></article>`).join('')||'<div class="empty">No decisions recorded yet.</div>'}load();setInterval(load,3500);</script></body></html>'''
